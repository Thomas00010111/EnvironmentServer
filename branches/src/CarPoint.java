 ...
 * Idea of this class was to make a car which is only a simple point.
 * Instead of writing/changes methods the functionality could be achieved
 * by LENGTHEDGE = 1
 */

public class CarPoint implements ICar{
	
	IAction actions = new ActionSimpleContinuous();
	...
	private final int LENGTHEDGE = 1;		//Car is only a dot
	...
	//reduced to 2 see comment in getBorders()
	private XYCoordinate[][] borders = new XYCoordinate[2][LENGTHEDGE]; 

	public CarPoint( Environment env) 
	{
	...
	/**
	 * Execute an Action and decrement the energy level of the car by <i>costsOneStep</i>. The action starts
	...
	public AngleXYCoordinate executeAction(int a, double speedFactor)
	{
		...
		deltaEnvironment = actions.execute(a, speedFactor, environment.getOrientation());
